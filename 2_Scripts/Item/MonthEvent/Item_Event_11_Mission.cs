...
public class Item_Event_11_Mission : ObjMng
{
    [Serializable]
    public struct SUI
	{
		...
		public Transform Element;		//Item_Event_11_Mission_Element
	}
	...
	public void SetData(MyFAEvent _event) {
		...
		List<MissionData> missions = new List<MissionData>();//USERINFO.m_Mission.Get_Missions(MissionMode.Event_StageMission);
		for(int i = 0;i< m_GrowUp.Missions.Count; i++) {
			missions.Add(USERINFO.m_Mission.Get_Mission(m_GrowUp.Missions[i].Idx));
		}
		...
		for(int i = 0; i < missions.Count; i++) {
			Item_Event_11_Mission_Element element = m_SUI.Bucket.GetChild(i).GetComponent<Item_Event_11_Mission_Element>();

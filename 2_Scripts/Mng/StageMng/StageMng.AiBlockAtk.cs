...
public class AiBlockAtkInfo
{
	public AiBlockAtkMode m_Type;
	...
	public AiBlockAtkInfo(AiBlockAtkMode type)
	{
		m_Type = type;
	}
	...
	public bool ISPosMoveType()
	{
		switch (m_Type)
		{
		case AiBlockAtkMode.AREA1:
		case AiBlockAtkMode.AREA3:
		case AiBlockAtkMode.Line:
			return true;
		}
		return false;
	}
	...
	public bool Equals(AiBlockAtkInfo info)
	{
...
public partial class StageMng : ObjMng
{
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Light
	public List<AiBlockAtkInfo> m_AiBlockAtkInfos = new List<AiBlockAtkInfo>();
	public void RemoveAiBlockAtkInfoAll()
	{
		for (int i = m_AiBlockAtkInfos.Count - 1; i > -1; i--) RemoveAiBlockAtkInfo(m_AiBlockAtkInfos[i]);

	}
	public void RemoveAiBlockAtkInfo(AiBlockAtkInfo info)
	{
		m_AiBlockAtkInfos.Remove(info);
		if (info.m_Eff) Destroy(info.m_Eff);
	}

	public void AddAiBlockAtkInfo(AiBlockAtkInfo info)
	{
		for (int i = m_AiBlockAtkInfos.Count - 1; i > -1; i--)
		{
			AiBlockAtkInfo temp = m_AiBlockAtkInfos[i];
			if (temp.Equals(info)) RemoveAiBlockAtkInfo(temp);

		}
		m_AiBlockAtkInfos.Add(info);
	}

	public bool IS_AiBlockAtkInfoCard(Item_Stage card)
	{
		for (int i = m_AiBlockAtkInfos.Count - 1; i > -1; i--)
		{
			if (m_AiBlockAtkInfos[i].IsAreaCard(card)) return true;
		}
		return false;
	}

	public void CheckAiBlockAtkInfoTurn(int selectpos)
	{
		for (int i = m_AiBlockAtkInfos.Count - 1; i > -1; i--)
		{
			AiBlockAtkInfo info = m_AiBlockAtkInfos[i];
			if (info.TurnCheck(selectpos)) RemoveAiBlockAtkInfo(info);
			else if (info.m_Line <= 0) RemoveAiBlockAtkInfo(info);
			else if (info.m_Pos < 0 || info.m_Pos >= m_ViewCard[info.m_Line].Length) RemoveAiBlockAtkInfo(info);
		}
	}
}

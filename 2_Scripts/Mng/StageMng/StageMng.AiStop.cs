...
public class AIStopInfo
{
	public AiStopMode m_Type;
	...
	public AIStopInfo(AiStopMode type)
	{
		m_Type = type;
	}
	...
	public bool ISPosMoveType()
	{
		switch (m_Type)
		{
		case AiStopMode.AREA1:
		case AiStopMode.AREA3:
		case AiStopMode.Line:
			return true;
		}
		return false;
	}
	...
	public bool Equals(AIStopInfo info)
	{
...
public partial class StageMng : ObjMng
{
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Light
	public List<AIStopInfo> m_AIStopInfos = new List<AIStopInfo>();
	public void RemoveAIStopAll()
	{
		for (int i = m_AIStopInfos.Count - 1; i > -1; i--) RemoveAIStopInfo(m_AIStopInfos[i]);

	}
	public void RemoveAIStopInfo(AIStopInfo info)
	{
		m_AIStopInfos.Remove(info);
		if (info.m_Eff) Destroy(info.m_Eff);
	}

	public void AddAIStopInfo(AIStopInfo info)
	{
		for (int i = m_AIStopInfos.Count - 1; i > -1; i--)
		{
			AIStopInfo temp = m_AIStopInfos[i];
			if (temp.Equals(info)) RemoveAIStopInfo(temp);

		}
		m_AIStopInfos.Add(info);
	}

	public bool IS_AIStopInfoCard(Item_Stage card)
	{
		for (int i = m_AIStopInfos.Count - 1; i > -1; i--)
		{
			if (m_AIStopInfos[i].IsAreaCard(card)) return true;
		}
		return false;
	}

	public void CheckAIStopInfoTurn(int selectpos)
	{
		for (int i = m_AIStopInfos.Count - 1; i > -1; i--)
		{
			AIStopInfo info = m_AIStopInfos[i];
			if (info.TurnCheck(selectpos)) RemoveAIStopInfo(info);
			else if (info.m_Line <= 0) RemoveAIStopInfo(info);
			else if (info.m_Pos < 0 || info.m_Pos >= m_ViewCard[info.m_Line].Length) RemoveAIStopInfo(info);
		}
	}
}

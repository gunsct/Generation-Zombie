...
public class Union_NoticeEdit : PopupBase
{
    [Serializable]
    public struct SUI
	{
	...
	GUILD_INFO_CHANGE_MODE m_SendMode;

	public int GetMaxLength()
	{
		return BaseValue.GUILD_NOTICE_LENGTH;
	}

	public override void SetData(PopupPos pos, PopupName popup, Action<int, GameObject> cb, object[] aobjValue) {

		m_SUI.Input.lineType = InputField.LineType.MultiLineSubmit;
		m_SUI.Input.characterLimit = GetMaxLength();
		((Text)m_SUI.Input.placeholder).text = TDATA.GetString(6041);
		m_SUI.Input.text = USERINFO.m_Guild.Notice;
		m_SUI.Title.text = TDATA.GetString(6040);
		m_SUI.Info.text = TDATA.GetString(6041);
		m_SendMode = GUILD_INFO_CHANGE_MODE.Notice;
